...
?>
    ...
    <?php
    $result = mysqli_query($conn,"SELECT * FROM objetivos where idusuario='$iduser'");
    while ($row = mysqli_fetch_assoc($result)) {
      ?>
          <div class="form-group col-md-12 col-xs-12 col-lg-12 ">
            <?php $codOBJETIVO = $row['id']; ?>
          <div class="objcadas">
              <div class="objdesc" idusuario= "<?php echo $iduser ?>"><?php echo $row['objetivo'] ?> | <a href="#" codObjetivo="<?php echo $codOBJETIVO; ?>" id="deletarobj" class="text-right" >Apagar?</a></div>
          </div>
          </div>
      <?php
        } ?>
